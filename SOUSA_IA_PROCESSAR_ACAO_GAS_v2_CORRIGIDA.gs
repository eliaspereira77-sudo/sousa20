/**
 * SOUSA 2.0 — SOUSA IA: PROCESSADOR CENTRAL DE AÇÕES (v2 — CORRIGIDA)
 * ==========================================================
 * ESTA É A VERSÃO CORRETA. Substitui SOUSA_IA_PROCESSAR_ACAO_GAS.gs
 * (v1), que assumia nomes de chave incorretos.
 *
 ...
 * no Cofre (via SOUSA_APIS_CASCATA + obterChaveAPI, camada que
 * já existe no projeto). Cada IA é um "anel de poder"; a união
 * forma a identidade própria da SOUSA IA — o "DNA" dela é a
 * soma dessas peças.
 *
 * PAPEL DESTE ARQUIVO:
 ...
 *   - Não duplica leitura do Cofre — reusa SOUSA_APIS_CASCATA e
 *     obterChaveAPI(), já usados em SOUSA_API_MANAGER.js e
 *     SOUSA_API_USB.js
 *
 ...
 * de cada "anel de poder" (provedor), para compor a identidade da
 * SOUSA IA.
 *
 * IMPORTANTE: esta função NÃO reimplementa a leitura do Cofre.
 * Ela reusa o que já existe (Zero Duplicidade):
 *   - SOUSA_APIS_CASCATA        → catálogo real de provedores
 *   - obterChaveAPI(nomeChave)  → resolve a chave real de cada um
 * Se algum dia SOUSA_APIS_CASCATA mudar de nome de chave, esta
 * função continua correta automaticamente — ela não guarda nomes
 * de chave hardcoded.
 */
function SOUSA_IA_montarDNA_() {
  if (typeof SOUSA_APIS_CASCATA === 'undefined') {
    return {
      ...
      total_conectado: 0,
      total_conhecido: 0,
      aviso: 'SOUSA_APIS_CASCATA não encontrada neste projeto — ' +
        'verifique se o arquivo de configuração de APIs está carregado.'
    };
  }

  var aneis = SOUSA_APIS_CASCATA.map(function(api) {
    var nomeChave = api.api_key || api.chave || null;
    var presente = nomeChave ? !!obterChaveAPI(nomeChave) : true; // sem chave = recurso local, sempre presente
    return {
      provedor: api.nome,
      chave: nomeChave,
      presente: presente,
      status_cadastro: api.status || 'DESCONHECIDO'
    };
  });
